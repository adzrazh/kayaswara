...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_milestone_status') {
        ...
        try {
            ...
            // WhatsApp — kirim saat in_progress dan completed
            ...
            // Email — kirim hanya saat milestone selesai (completed)
            $email_result = ['success' => false, 'message' => 'skip'];
            if ($new_status === 'completed' && !empty($order['client_email'])) {
                $email_result = sendMilestoneEmail($order, $ms_title, $milestone_id);
            }
            ...
            if ($email_result['success']) {
                $flash_msg .= ' Notifikasi email terkirim.';
            }
...
<div class="admin-content">
    ...
    <div class="page-header">
        ...
        <div class="page-header-actions">
            ...
            <?php
            $existingInvoice = fetch("SELECT id, status, invoice_number FROM invoices WHERE order_id = ? AND status != 'cancelled' ORDER BY id DESC LIMIT 1", [$id]);
            if ($existingInvoice): ?>
            <a href="index.php?page=invoice-detail&id=<?= $existingInvoice['id'] ?>" class="btn btn-<?= $existingInvoice['status'] === 'paid' ? 'success' : 'warning' ?>">
                <i class="fas fa-file-invoice-dollar me-1"></i>Invoice <?= htmlspecialchars($existingInvoice['invoice_number']) ?>
            </a>
            <?php else: ?>
            <a href="index.php?page=invoice-form&order_id=<?= $id ?>" class="btn btn-primary">
            ...
            <div class="admin-card mb-4">
                ...
                <div class="admin-card-body">
                    ...
                    if (!empty($order['addons'])) {
                        $detailAddons = json_decode($order['addons'], true) ?: [];
                    }
                    ?>
                    ...
                    <div style="margin-top:16px;">
                        ...
                        <div style="background:#f0fdf4;border-radius:10px;padding:14px;border-left:4px solid #22c55e;font-size:13.5px;">
                            <?php
                            $addonsGrandTotal = 0;
                            foreach ($detailAddons as $da):
                                $addonsGrandTotal += (int)($da['price'] ?? 0);
                            ?>
                            <div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid #dcfce7;">
                                <span style="color:#374151;">+ <?= htmlspecialchars($da['name']) ?></span>
                                <span style="font-weight:600;color:#15803d;">Rp <?= number_format((int)($da['price'] ?? 0), 0, ',', '.') ?></span>
                            </div>
                            <?php endforeach; ?>
                            <?php if (!empty($order['price'])): ?>
                            <div style="display:flex;justify-content:space-between;padding:6px 0 2px;font-weight:700;color:#1e293b;">
        ...
        <div class="col-xl-4">
            ...
            <div class="admin-card mb-4" style="background:linear-gradient(135deg,rgba(26,60,94,0.04),rgba(26,54,93,0.04));">
                <div class="admin-card-body">
                    ...
                    <div style="display:flex;flex-direction:column;gap:10px;font-size:13px;">
                        ...
                        <div style="display:flex;justify-content:space-between;">
                            <span style="color:#64748b;">Nilai Pesanan:</span>
                            <strong style="color:#1A1A1A;">
                                ...
                                if (!empty($detailAddons)) echo ' <small style="font-size:10px;color:#94a3b8;">('.count($detailAddons).' add-on)</small>';
                                ?>
                            </strong>
                        </div>
                        <?php endif; ?>
            ...
            <div class="admin-card mb-4">
                ...
                <div class="admin-card-body">
                    ...
                    <?php if (!empty($order['client_email'])): ?>
                    <a href="mailto:<?= htmlspecialchars($order['client_email']) ?>?subject=Update%20Pesanan%20<?= urlencode($order['tracking_code']) ?>&body=Halo%20<?= urlencode($order['client_name']) ?>,%0A%0ABerikut%20update%20mengenai%20pesanan%20Anda%20dengan%20kode%20<?= urlencode($order['tracking_code']) ?>.%0A%0ASalam,%0ATim%20Kayaswara"
                    ...
                    ?>
                    <a href="https://wa.me/<?= $wa2 ?>?text=Halo%20<?= urlencode($order['client_name']) ?>,%20kami%20dari%20Kayaswara%20ingin%20memberikan%20update%20pesanan%20Anda%20(<?= urlencode($order['tracking_code']) ?>)."
                       target="_blank"
