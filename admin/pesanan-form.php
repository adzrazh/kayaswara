...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Parse add-ons
    $addons_raw  = trim($_POST['addons_json'] ?? '');
    $addons_list = [];
    if (!empty($addons_raw)) {
        $decoded = json_decode($addons_raw, true);
        if (is_array($decoded)) {
            foreach ($decoded as $a) {
                $aname  = trim($a['name'] ?? '');
                $aprice = max(0, (int)($a['price'] ?? 0));
                if (!empty($aname)) {
                    $addons_list[] = ['name' => $aname, 'price' => $aprice];
                }
            }
        }
    }
        ...
        try {
            if ($is_edit) {
                ...
                if (!empty($order['client_email'])) {
                    ...
                    $tracking_url  = (defined('SITE_URL') ? rtrim(SITE_URL, '/') : '') . '/tracking';
                    $email_subject = "[{$site_name}] Konfirmasi Pesanan – Kode: {$tracking_code}";

                    $email_body  = "Yth. {$order['client_name']},\r\n\r\n";
                    $email_body .= "Terima kasih telah mempercayakan kebutuhan penerbitan buku Anda kepada kami.\r\n";
                    $email_body .= "Pesanan Anda telah berhasil dibuat dengan detail berikut:\r\n\r\n";
                    $email_body .= "====================================\r\n";
                    $email_body .= " DETAIL PESANAN\r\n";
                    $email_body .= "====================================\r\n";
                    $email_body .= "Kode Tracking : {$tracking_code}\r\n";
                    $email_body .= "Layanan       : {$service_label}\r\n";
                    $email_body .= "Paket         : {$package_label}\r\n";
                    if (!empty($order['client_institution'])) {
                        $email_body .= "Institusi     : {$order['client_institution']}\r\n";
                    }
                    if (!empty($order['client_phone'])) {
                        $email_body .= "No. HP/WA     : {$order['client_phone']}\r\n";
                    }
                    if (!empty($order['price'])) {
                        $email_body .= "Harga Paket   : Rp " . number_format((int)$order['price'], 0, ',', '.') . "\r\n";
                    }
                    if (!empty($addons_list)) {
                        $email_body .= "\r\nLayanan Tambahan (Add-ons):\r\n";
                        $addons_total = 0;
                        foreach ($addons_list as $a) {
                            $email_body .= "  + " . $a['name'] . " : Rp " . number_format($a['price'], 0, ',', '.') . "\r\n";
                            $addons_total += $a['price'];
                        }
                        $total_order = (int)$order['price'] + $addons_total;
                        $email_body .= "Total Estimasi: Rp " . number_format($total_order, 0, ',', '.') . "\r\n";
                    }
                    $email_body .= "Status        : Menunggu\r\n";
                    if (!empty($order['description'])) {
                        $email_body .= "\r\nDeskripsi Kebutuhan:\r\n";
                        $email_body .= "------------------------------------\r\n";
                        $email_body .= strip_tags($order['description']) . "\r\n";
                        $email_body .= "------------------------------------\r\n";
                    }
                    $email_body .= "\r\nPantau perkembangan pesanan Anda secara real-time di:\r\n";
                    $email_body .= "{$tracking_url}\r\n\r\n";
                    $email_body .= "Masukkan kode tracking <{$tracking_code}> untuk melihat detail.\r\n\r\n";
                    $email_body .= "Tim kami akan segera menghubungi Anda untuk konfirmasi lebih lanjut.\r\n\r\n";
                    $email_body .= "Hormat kami,\r\n{$site_name}";

                    sendSiteEmail($order['client_email'], $email_subject, $email_body);
                }
            ...
            <div class="col-xl-8">
                ...
                <div class="admin-card mb-4">
                    ...
                    <div class="admin-card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                <label class="form-label">Layanan Tambahan <small style="font-size:11px;color:#9AA7B2;">(opsional — tampil di invoice sebagai baris terpisah)</small></label>
                                <div id="addonRows" style="display:flex;flex-direction:column;gap:8px;margin-bottom:8px;">
                                    <?php
                                    $existingAddons = [];
                                    if (!empty($order['addons'])) {
                                        $existingAddons = json_decode($order['addons'], true) ?: [];
                                    }
                                    foreach ($existingAddons as $ai => $av): ?>
                                    <div class="addon-row d-flex gap-2 align-items-center" data-idx="<?= $ai ?>">
                                        <input type="text" class="form-control addon-name" placeholder="Nama layanan tambahan"
                                               value="<?= htmlspecialchars($av['name']) ?>" style="flex:2;">
                                        <div class="input-group" style="flex:1;">
                                            <span class="input-group-text">Rp</span>
                                            <input type="text" class="form-control addon-price" placeholder="0"
                                                   value="<?= number_format((int)$av['price'], 0, ',', '.') ?>"
                                                   oninput="formatAddonPrice(this); syncAddons()">
                                        </div>
                                        <button type="button" class="btn btn-outline-danger btn-sm" onclick="removeAddonRow(this)">
                                            <i class="fas fa-times"></i>
                                        </button>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                                <button type="button" class="btn btn-outline-secondary btn-sm" onclick="addAddonRow()">
                                    <i class="fas fa-plus me-1"></i>Tambah Add-on
                                </button>
                                <input type="hidden" name="addons_json" id="addons_json" value="<?= htmlspecialchars($order['addons'] ?? '[]') ?>">
                                <div class="form-hint mt-1">Contoh: Cek Plagiasi, Cetak Tambahan, E-Book, Konsultasi Lanjutan.</div>
                            </div>
                            <div class="col-12">
                ...
                <div class="admin-card">
                    ...
                    <div class="admin-card-body">
                        ...
                        <div style="font-size:12px;color:#9AA7B2;margin-top:4px;">
            ...
            <div class="col-xl-4">
                ...
                <div class="admin-card mb-4">
                    ...
                    <div class="admin-card-body">
                        ...
                        <div style="background:#F9FBFD;border-radius:8px;padding:12px;border:1px solid #DCE5ED;">
                            <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#9AA7B2;margin-bottom:6px;">
                                Kode Tracking
                            </div>
                            <code style="font-size:13px;font-weight:700;color:#12212E;">
                        ...
                        <div style="background:#f0fdf4;border-radius:8px;padding:12px;border:1px solid #bbf7d0;">
                            ...
                            <div style="font-size:11px;color:#9AA7B2;margin-top:4px;">
                ...
                <div class="admin-card mb-4" style="background:linear-gradient(135deg,rgba(26,60,94,0.04),rgba(26,54,93,0.04));">
                    <div class="admin-card-body">
                        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#9AA7B2;margin-bottom:12px;">
                            <i class="fas fa-info-circle me-1"></i>Informasi
                        </div>
                        <div style="font-size:13px;color:#41525F;line-height:1.6;">
                            ...
                            <ul class="mb-0 ps-3" style="font-size:12.5px;">
                                ...
                                <li>Mengirim email konfirmasi ke klien (jika email diisi)</li>
                            </ul>
                        </div>
                    </div>
                </div>
...
<script>
// ---------- Add-on rows ----------
function formatAddonPrice(el) {
    var raw = el.value.replace(/[^0-9]/g, '');
    el.value = raw ? parseInt(raw, 10).toLocaleString('id-ID') : '';
}
function syncAddons() {
    var rows  = document.querySelectorAll('#addonRows .addon-row');
    var items = [];
    rows.forEach(function(row) {
        var name  = row.querySelector('.addon-name').value.trim();
        var priceRaw = row.querySelector('.addon-price').value.replace(/[^0-9]/g, '');
        var price = parseInt(priceRaw, 10) || 0;
        if (name) items.push({name: name, price: price});
    });
    document.getElementById('addons_json').value = JSON.stringify(items);
}
function addAddonRow() {
    var container = document.getElementById('addonRows');
    var idx = container.children.length;
    var div = document.createElement('div');
    div.className = 'addon-row d-flex gap-2 align-items-center';
    div.setAttribute('data-idx', idx);
    div.innerHTML =
        '<input type="text" class="form-control addon-name" placeholder="Nama layanan tambahan" oninput="syncAddons()" style="flex:2;">' +
        '<div class="input-group" style="flex:1;">' +
            '<span class="input-group-text">Rp</span>' +
            '<input type="text" class="form-control addon-price" placeholder="0"' +
                   ' oninput="formatAddonPrice(this); syncAddons()" inputmode="numeric">' +
        '</div>' +
        '<button type="button" class="btn btn-outline-danger btn-sm" onclick="removeAddonRow(this)">' +
            '<i class="fas fa-times"></i>' +
        '</button>';
    container.appendChild(div);
    div.querySelector('.addon-name').focus();
}
function removeAddonRow(btn) {
    btn.closest('.addon-row').remove();
    syncAddons();
}
// Sync existing rows on name input too
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('#addonRows .addon-name').forEach(function(el) {
        el.addEventListener('input', syncAddons);
    });
    syncAddons();
    // Pre-submit: ensure addons_json is current
