<?php
  ...
  // Block web access — only allow CLI or local server
  $allowedIps = ['127.0.0.1', '::1'];
  $remoteAddr = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
  if (PHP_SAPI !== 'cli' && !in_array($remoteAddr, $allowedIps)) {
      http_response_code(403);
      exit('Akses ditolak. Script ini hanya dapat dijalankan dari localhost atau CLI.');
  }
  ...
  <body>
      <h1>Kayaswara &mdash; Database Migration</h1>
      <ul>
